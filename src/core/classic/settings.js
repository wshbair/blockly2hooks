 ...
 *
 * @fileoverview JavaScript for Blockly2hook Server Compiler settings.
 */
'use strict';
...
window.addEventListener('load', function() {
  ...
  Blockly2hookServer.requestCompilerLocation(
      ArduinoSettings.setCompilerLocationHtml);
  Blockly2hookServer.requestSketchLocation(
      ArduinoSettings.setSketchLocationHtml);
  Blockly2hookServer.requestArduinoBoards(
      ArduinoSettings.setArduinoBoardsHtml);
  Blockly2hookServer.requestSerialPorts(ArduinoSettings.setSerialPortsHtml);
  Blockly2hookServer.requestIdeOptions(ArduinoSettings.setIdeHtml);

  // Binding clicks to the form items
  ArduinoSettings.bindClick_('settings_compiler_location', function() {
    Blockly2hookServer.requestNewCompilerLocation(
        ArduinoSettings.setCompilerLocationHtml);
  });
  ArduinoSettings.bindClick_('settings_sketch_location', function() {
    Blockly2hookServer.requestNewSketchLocation(
        ArduinoSettings.setSketchLocationHtml);
  });
...
ArduinoSettings.setCompilerLocationHtml = function(jsonResponse) {
  if (jsonResponse != null) {
    var newEl = Blockly2hookServer.createElementFromJson(jsonResponse);
...
ArduinoSettings.setBoard = function() {
  ...
  //TODO: check how Blockly2hookServer deals with invalid data and sanitise here
  Blockly2hookServer.setArduinoBoard(
...
ArduinoSettings.setSerial = function() {
  ...
  //TODO: check how Blockly2hookServer deals with invalid data and sanitise
  Blockly2hookServer.setSerialPort(
...
ArduinoSettings.setIdeSettings = function() {
  ...
  //TODO: check how Blockly2hookServer deals with invalid data and sanitise
  Blockly2hookServer.setIdeOptions(ideValue, ArduinoSettings.setIdeHtml);
};
