...
def _create_qqq_file(output_dir, Blockly2hook=False):
    ...
    """
    qqq_file_name = 'qqq_Blockly2hook.json' if Blockly2hook else 'qqq.json'
...
def _create_lang_file(author, lang, output_dir, Blockly2hook=False):
    ...
    Raises:
        IOError: An error occurred while opening or writing the file.
    """
    qqq_name = 'qqq' + ('_Blockly2hook' if Blockly2hook else '')
    lang_file_name = lang + ('_Blockly2hook.json' if Blockly2hook else '.json')
...
def write_files_Blockly2hook(author, lang, output_dir, units):
    """
    Same as write_files, but does not include the key_files and generates
    lang_Blockly2hook.json and qqq_Blockly2hook.json instead.
    """
    lang_file = _create_lang_file(author, lang, output_dir, Blockly2hook=True)
    qqq_file = _create_qqq_file(output_dir, Blockly2hook=True)
    first_entry = True
    for unit in units:
        if not first_entry:
            lang_file.write(',\n')
