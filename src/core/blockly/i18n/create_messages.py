...
def main():
  ...
  parser.add_argument('--Blockly2hook', action='store_true', default=False,
                      help='Attach Blockly2hook strings to lang js files')
  ...
  for arg_file in args.files:
    ...
    if target_lang not in ('qqq', 'keys', 'synonyms'):
      if args.Blockly2hook:
        # For Blockly2hook pass search for {lang}_Blockly2hook.json target file
        target_defs = {}
        ardu_json = os.path.join(dir_, target_lang + '_Blockly2hook.json')
        if os.path.isfile(ardu_json):
          target_defs = read_json_file(os.path.join(os.curdir, ardu_json))
          print(u'Processed Blockly2hook translation: {0}'.format(ardu_json))
      else:
      ...
      # Prepare differences from parsing lang.json and lang_Blockly2hook.json
      if args.Blockly2hook:
        file_write_mode = 'a'
        start_str = '\n\n// Blockly2hook strings\n'
      else:
        file_write_mode = 'w'
