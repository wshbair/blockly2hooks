...
import os
import re
from common import write_files, write_files_Blockly2hook
...
def main():
  ...
  parser.add_argument('--Blockly2hook', action='store_true', default=False,
                      help='Create files for Blockly2hook messages')
  ...
  if args.Blockly2hook:
    write_files_Blockly2hook(args.author, args.lang, args.output_dir, results)
  else:
    write_files(args.author, args.lang, args.output_dir, results, False)

  # Create synonyms.json.
  synonym_name = 'synonyms' + \
                 ('_Blockly2hook.json' if args.Blockly2hook else '.json')
