...
class Gen_langfiles(threading.Thread):
  ...
  def generate_Blockly2hook(self):
    """
    Recreates the msg/json/{en,qqq,synonyms}_Blockly2hook.json files if older
    than msg/messages_Blockly2hook.js.
    Then attaches the {lang}_Blockly2hook.json strings into the {lang}.js files.
    """
    # The files msg/json/{en,qqq,synonyms}.json depend on msg/messages.js.
    if self._rebuild([os.path.join("msg", "messages_Blockly2hook.js")],
                     [os.path.join("msg", "json", f) for f in
                      ["en_Blockly2hook.json",
                       "qqq_Blockly2hook.json",
                       "synonyms_Blockly2hook.json"]]):
      try:
        subprocess.check_call([
            "python",
            os.path.join("i18n", "js_to_json.py"),
            "--author", "carlosperate",
            "--input_file", "msg/messages_Blockly2hook.js",
            "--output_dir", "msg/json/",
            "--Blockly2hook",
            "--quiet"])
      except (subprocess.CalledProcessError, OSError) as e:
        print("Error running i18n/js_to_json.py for Blockly2hook pass: ", e)
        sys.exit(1)

    try:
      # Use create_messages.py to attach _Blockly2hook.json strings to .js files
      cmd = [
          "python",
          os.path.join("i18n", "create_messages.py"),
      ...
      print("Error running i18n/create_messages.py for Blockly2hook pass: ", e)
      sys.exit(1)

  def run(self):
    """
    Runs the thread to generate the Blockly JavaScript string files and append
    the Blockly2hook strings at the end as well.
    """
    self.generate_blockly()
    # Blockly2hook appends it's strings at the end of blockly output
    self.generate_Blockly2hook()
