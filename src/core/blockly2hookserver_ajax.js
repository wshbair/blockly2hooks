 ...
 *
 * @fileoverview Ajax calls to the Blockly2hook Server.
 */
'use strict';

/** Create a name space for the application. */
var Blockly2hookServer = {};
 ...
 */
Blockly2hookServer.getJson = function(url, callback) {
  Blockly2hookServer.sendRequest(url, 'GET', 'application/json', null, callback);
};

/**
 * Sends JSON data to the Blockly2hookServer.
 * @param {!string} url Requestor URL.
 * @param {!string} json JSON string.
 * @param {!function} callback Request callback function.
 */
Blockly2hookServer.putJson = function(url, json, callback) {
  Blockly2hookServer.sendRequest(url, 'PUT', 'application/json', json, callback);
};

/**
 * Sends a request to the Remote  Server that returns a JSON response.
 * @param {!string} url Requestor URL.
 ...
 *     parsed JSON object.
 */
Blockly2hookServer.sendRequest = function(
...
Blockly2hookServer.createRequest = function() {
  var request = null;
  try {
    ...
    try {
      ...
      try {
        ...
        throw 'Your browser does not support AJAX. You will not be able to' +
              'use all of Blockly2hook features.';
        request = null;
      }
    }
  }
...
Blockly2hookServer.jsonToHtmlTextInput = function(jsonObj) {
  var element = null;
  if (jsonObj) {
    // Simple text input
...
Blockly2hookServer.jsonToHtmlDropdown = function(jsonObj) {
  var element = null;
  if (!jsonObj) {
 ...
 */
Blockly2hookServer.setSketchLocation = function(new_path, callback) {
  Blockly2hookServer.putJson(
...
/**
 * Gets the current IDE setting from the Blockly2hook Server settings. The new
 * settings menu for the IDE options is then processed into an HTML element
 * and sent to the callback function as an argument.
 * @param {!function} callback Callback function for the server request, must
 *     have one argument to receive the JSON response.
 */
Blockly2hookServer.requestIdeOptions = function(callback) {
  //Blockly2hookServer.getJson('/settings/ide', callback);
};

/**
 * Sends the inputted IDE option to the Blockly2hook Server Settings. The new
 * settings menu for the IDE options is then processed into an HTML element
 * and sent to the callback function as an argument.
 ...
 *     have one argument to receive the JSON response.
 */
Blockly2hookServer.setIdeOptions = function(ide_option, callback) {
  Blockly2hookServer.putJson(
 ...
 *     have one argument to receive the JSON response.
 */
Blockly2hookServer.sendHookToCompile = function(code, callback) {
    code = Blockly2hookServer.getHookApiFile()+ Blockly2hookServer.getSfCodesFile()+ Blockly2hookServer.getHookMacroFile()+ code;
    ...
    xhr.addEventListener("load", function () {
      ...
      console.log(getAnnotations(wast))
      if (!getAnnotations(wast).length)
        callback(wast, 0);
        else
        callback(wast,1)
    });  
...
Blockly2hookServer.hookDeploy = function(wasmBinaryFile, walletAddress, callback){
...
/**
* Publish the hook to the XRPL ledger
*/
Blockly2hookServer.publishHook = function(setHookJson, callback){
...
Blockly2hookServer.runAcceptTest = function(callback){
    var xhr = new XMLHttpRequest();
    xhr.addEventListener("load", function () {
      var response = this.responseText;
      callback(response);
...
Blockly2hookServer.getHookApiFile = function(){
  var header = `/**
...
Blockly2hookServer.getHookMacroFile = function(){
  const Macro = `/**
...
Blockly2hookServer.getSfCodesFile = function(){
  const codes =`/**
...
function getAnnotations(output) {
    var annotations = [];
    if (!output) {
      return annotations;
    }
