...
goog.DEPENDENCIES_ENABLED && (goog.included_ = {}, goog.dependencies_ = {
    ...
    for (f = 0; f < b.length; f++)
        if (e = b[f]) d.pathIsModule[e] ? goog.importModule_(goog.basePath + e) : goog.importScript_(goog.basePath + e);
...
goog.retrieveAndExecModule_ = function(a) {
    if (!COMPILED) {
        ...
        e.send();
        d = e.responseText;
        if (null != d) e = goog.wrapModule_(a, d), goog.IS_OLD_IE_ ? (goog.dependencies_.deferred[b] = e, goog.queuedModules_.push(b)) : c(a, e);
    ...
    for (var c = 0, d = goog.string.trim(String(a)).split("."), e = goog.string.trim(String(b)).split("."), f = Math.max(d.length, e.length), g = 0; 0 == c && g < f; g++) {
        ...
        do {
            ...
            var c = 0 == n[1].length ? 0 : parseInt(n[1], 10),
                ...
                c = goog.string.compareElements_(c, p) || goog.string.compareElements_(0 == n[2].length, 0 == q[2].length) ||
                goog.string.compareElements_(n[2], q[2])
        } while (0 == c)
    }
    return c
};
...
goog.labs.userAgent.browser.getIEVersion_ = function(a) {
    var b = /rv: *([\d\.]*)/.exec(a);
    if (b && b[1] || (b = /Edge\/([\d\.]+)/.exec(a))) return b[1];
    var b = "",
        c = /MSIE +([\d\.]+)/.exec(a);
    if (c && c[1])
...
goog.Promise = function(a, b) {
    ...
    if (a == goog.Promise.RESOLVE_FAST_PATH_) this.resolve_(goog.Promise.State_.FULFILLED,
        b);
    else try {
        var c = this;
        a.call(b, function(a) {
...
goog.Promise.race = function(a) {
    return new goog.Promise(function(b, c) {
        a.length || b(void 0);
        for (var d = 0, e; e = a[d]; d++) goog.Promise.maybeThenVoid_(e, b, c)
    })
};
    ...
    return new goog.Promise(function(b, c) {
        ...
        if (d)
            for (var f = function(a, c) {
                    ...
                    c(a)
                }, h = 0, k; k = a[h]; h++) goog.Promise.maybeThenVoid_(k, goog.partial(f, h), g);
        else b(e)
    })
};
    ...
    return new goog.Promise(function(b, c) {
        ...
        if (d)
            for (var f = function(a) {
                    ...
                    e[a] = b;
                    0 == d && c(e)
                }, h = 0, k; k = a[h]; h++) goog.Promise.maybeThenVoid_(k, f, goog.partial(g, h));
        else b(void 0)
    })
};
...
goog.Promise.maybeThenVoid_ = function(a, b, c, d) {
    a instanceof goog.Promise ? a.thenVoid(b, c, d) : a.then(b, c, d)
};
goog.Promise.prototype.thenAlways = function(a, b) {
    ...
    var c = function() {
        ...
        c = goog.Promise.getCallbackEntry_(c, c, null);
    c.always = !0;
...
goog.Promise.prototype.resolve_ = function(a, b) {
    if (this.state_ == goog.Promise.State_.PENDING) {
        if (this == b) a = goog.Promise.State_.REJECTED, b = new TypeError("Promise cannot resolve to itself");
        else {
            if (goog.Thenable.isImplementedBy(b)) {
                this.state_ = goog.Promise.State_.BLOCKED;
                goog.Promise.maybeThenVoid_(b, this.unblockAndFulfill_, this.unblockAndReject_, this);
                return
            }
            if (goog.isObject(b)) try {
                var c = b.then;
...
goog.Promise.prototype.executeCallback_ = function(a, b, c) {
    ...
    b == goog.Promise.State_.FULFILLED ? a.onFulfilled.call(a.context, c) : null != a.onRejected && (a.always || this.removeUnhandledRejection_(), a.onRejected.call(a.context, c));
...
goog.events.BrowserEvent.prototype.init = function(a, b) {
    var c = this.type = a.type,
        d = null;
    if (c == goog.events.EventType.TOUCHSTART || c == goog.events.EventType.TOUCHMOVE) d = a.targetTouches[0];
    else if (c == goog.events.EventType.TOUCHEND || c == goog.events.EventType.TOUCHCANCEL) d = a.changedTouches[0];
    this.target = goog.isNull(d) ? a.target || a.srcElement : d.target;
    this.currentTarget = b;
    var e = a.relatedTarget;
    e ? goog.userAgent.GECKO && (goog.reflect.canAccessProperty(e, "nodeName") || (e = null)) : c == goog.events.EventType.MOUSEOVER ? e =
        a.fromElement : c == goog.events.EventType.MOUSEOUT && (e = a.toElement);
...
goog.html.SafeHtml.getAttrNameAndValue_ = function(a, b, c) {
    if (c instanceof goog.string.Const) c = goog.string.Const.unwrap(c);
        ...
        else throw Error('Attribute "' + b + '" on tag "' + a + '" requires goog.html.SafeUrl or goog.string.Const value, "' + c + '" given.');
    }
...
Blockly.BlockSvg.prototype.renderCompute_ = function(a) {
    ...
    for (var d = 0, e = 0, f = !1, g = !1, h = !1, k = void 0, l = this.block_.inputsInline && !this.block_.isCollapsed(), m = 0, n; n = b[m]; m++)
        if (n.isVisible()) {
            var q;
            l && k && k != Blockly.NEXT_STATEMENT && n.type != Blockly.NEXT_STATEMENT ? q = c[c.length - 1] :
                (k = n.type, q = [], q.type = l && n.type != Blockly.NEXT_STATEMENT ? Blockly.BlockSvg.INLINE : n.type, q.height = 0, c.push(q));
            q.push(n);
            ...
            if (n.connection && n.connection.targetConnection) {
                var p = n.connection.targetBlock().getHeightWidth();
                n.renderHeight = Math.max(n.renderHeight, p.height);
                n.renderWidth = Math.max(n.renderWidth, p.width)
            }
            m == b.length - 1 && n.renderHeight--;
            q.height =
                Math.max(q.height, n.renderHeight);
            n.fieldWidth = 0;
            1 == c.length && (n.fieldWidth += Blockly.RTL ? -a : a);
            for (var p = !1, r = 0, t; t = n.fieldRow[r]; r++) {
            ...
            q.type != Blockly.BlockSvg.INLINE && (q.type == Blockly.NEXT_STATEMENT ? (g = !0, e = Math.max(e, n.fieldWidth)) : (q.type == Blockly.INPUT_VALUE ?
                f = !0 : q.type == Blockly.DUMMY_INPUT && (h = !0), d = Math.max(d, n.fieldWidth)))
        } for (a = 0; q = c[a]; a++)
...
Blockly.BlockSvg.prototype.renderDrawRight_ = function(a, b, c, d, e, f, g) {
    for (var h, k = 0, l, m, n = 0, q; q = f[n]; n++) {
        ...
        if (this.block_.isCollapsed()) {
            ...
            this.renderFields_(p.fieldRow, h, l);
        ...
        } else q.type == Blockly.INPUT_VALUE ? (p = q[0], l = k + Blockly.BlockSvg.FIELD_HEIGHT, p.align != Blockly.ALIGN_LEFT && (r = f.rightEdge - p.fieldWidth - Blockly.BlockSvg.TAB_WIDTH - 2 * Blockly.BlockSvg.SEP_SPACE_X, p.align == Blockly.ALIGN_RIGHT ? h += r : p.align == Blockly.ALIGN_CENTRE && (h += (r + h) / 2)), this.renderFields_(p.fieldRow,
            h, l), a.push(Blockly.BlockSvg.TAB_PATH_DOWN), r = q.height - Blockly.BlockSvg.TAB_HEIGHT, a.push("v", r), Blockly.RTL ? (b.push(Blockly.BlockSvg.TAB_PATH_DOWN_HIGHLIGHT_RTL), b.push("v", r)) : (b.push("M", f.rightEdge - 4.2 + "," + (k + Blockly.BlockSvg.TAB_HEIGHT - .4)), b.push("l", .42 * Blockly.BlockSvg.TAB_WIDTH + ",-1.8")), l = e.x + (Blockly.RTL ? -f.rightEdge - 1 : f.rightEdge + 1), m = e.y + k, p.connection.moveTo(l, m), p.connection.targetConnection && (p.connection.tighten_(), this.width = Math.max(this.width, f.rightEdge + p.connection.targetBlock().getHeightWidth().width -
            Blockly.BlockSvg.TAB_WIDTH + 1))) : q.type == Blockly.DUMMY_INPUT ? (p = q[0], l = k + Blockly.BlockSvg.FIELD_HEIGHT, p.align != Blockly.ALIGN_LEFT && (r = f.rightEdge - p.fieldWidth - 2 * Blockly.BlockSvg.SEP_SPACE_X, f.hasValue && (r -= Blockly.BlockSvg.TAB_WIDTH), p.align == Blockly.ALIGN_RIGHT ? h += r : p.align == Blockly.ALIGN_CENTRE && (h += (r + h) / 2)), this.renderFields_(p.fieldRow, h, l), a.push("v", q.height), Blockly.RTL && b.push("v", q.height - 2)) : q.type == Blockly.NEXT_STATEMENT && (p = q[0], 0 == n && (a.push("v", Blockly.BlockSvg.SEP_SPACE_Y), Blockly.RTL &&
                b.push("v", Blockly.BlockSvg.SEP_SPACE_Y - 1), k += Blockly.BlockSvg.SEP_SPACE_Y), l = k + Blockly.BlockSvg.FIELD_HEIGHT, p.align != Blockly.ALIGN_LEFT && (r = f.statementEdge - p.fieldWidth - 2 * Blockly.BlockSvg.SEP_SPACE_X, p.align == Blockly.ALIGN_RIGHT ? h += r : p.align == Blockly.ALIGN_CENTRE && (h += (r + h) / 2)), this.renderFields_(p.fieldRow, h, l), h = f.statementEdge + Blockly.BlockSvg.NOTCH_WIDTH, a.push("H", h), a.push(Blockly.BlockSvg.INNER_TOP_LEFT_CORNER), a.push("v", q.height - 2 * Blockly.BlockSvg.CORNER_RADIUS), a.push(Blockly.BlockSvg.INNER_BOTTOM_LEFT_CORNER),
            a.push("H", f.rightEdge), Blockly.RTL ? (b.push("M", h - Blockly.BlockSvg.NOTCH_WIDTH + Blockly.BlockSvg.DISTANCE_45_OUTSIDE + "," + (k + Blockly.BlockSvg.DISTANCE_45_OUTSIDE)), b.push(Blockly.BlockSvg.INNER_TOP_LEFT_CORNER_HIGHLIGHT_RTL), b.push("v", q.height - 2 * Blockly.BlockSvg.CORNER_RADIUS), b.push(Blockly.BlockSvg.INNER_BOTTOM_LEFT_CORNER_HIGHLIGHT_RTL), b.push("H", f.rightEdge - 1)) : (b.push("M", h - Blockly.BlockSvg.NOTCH_WIDTH + Blockly.BlockSvg.DISTANCE_45_OUTSIDE + "," + (k + q.height - Blockly.BlockSvg.DISTANCE_45_OUTSIDE)),
                b.push(Blockly.BlockSvg.INNER_BOTTOM_LEFT_CORNER_HIGHLIGHT_LTR), b.push("H", f.rightEdge)), l = e.x + (Blockly.RTL ? -h : h), m = e.y + k + 1, p.connection.moveTo(l, m), p.connection.targetConnection && (p.connection.tighten_(), this.width = Math.max(this.width, f.statementEdge + p.connection.targetBlock().getHeightWidth().width)), n == f.length - 1 || f[n + 1].type == Blockly.NEXT_STATEMENT) && (a.push("v", Blockly.BlockSvg.SEP_SPACE_Y), Blockly.RTL && b.push("v", Blockly.BlockSvg.SEP_SPACE_Y - 1), k += Blockly.BlockSvg.SEP_SPACE_Y);
        k += q.height
    }
    f.length ||
...
Blockly.Xml.domToWorkspace = function(a, b) {
    var c;
    Blockly.RTL && (c = a.getMetrics().viewWidth);
...
Blockly.Blocks.showResult = function(a) {
    function b(c, g) {
        for (var h = 0, k; k = c.childNodes[h]; h++)
            if (k.tagName) {
    ...
    var c = new Blockly.Toolbox.TreeControl(goog.html.SafeHtml.EMPTY, Blockly.Toolbox.CONFIG_);
...
Blockly.Blocks.setCheckVariable = function(a, c, b) {
    switch (c) {
        case "int":
            a.getInput(b).setCheck("Number Aster Array Boolean Macro Variable VAR_INT NEGATIVE INT".split(" "));
            break;
        case "unsigned int":
            a.getInput(b).setCheck("Number Aster Array Boolean Macro Variable VAR_UNINT NEGATIVE".split(" "));
            break;
        case "float":
            a.getInput(b).setCheck("Number Aster Array Boolean Macro Variable VAR_FLOAT DOUBLE".split(" "));
            break;
        case "double":
            a.getInput(b).setCheck("Number Aster Array Boolean Macro Variable VAR_DOUBLE DOUBLE".split(" "));
            break;
        case "char":
            a.getInput(b).setCheck("String Aster Array Boolean Macro Variable VAR_CHAR CHAR Number INT".split(" "))
    }
};
...
Blockly.Blocks.checkUnselect = function(a) {
    if ("___EC_84_A0_ED_83_9D__" == a || "--Select--" == a || "___ED_83_80_EC_9E_85__" == a || "--Type--" == a || "-Item-" == a || "-Type-" == a) a = "unselected";
    return a
};
// Copyright 2011 Google Inc.  Apache License 2.0
...
Blockly.Connection.prototype.closest = function(a, b, c) {
    function d(b) {
        var c = e[b];
        if ((c.type == Blockly.OUTPUT_VALUE || c.type == Blockly.PREVIOUS_STATEMENT) && c.targetConnection || c.type == Blockly.INPUT_VALUE && c.targetConnection && !c.targetBlock().isMovable() || !m.checkType_(c)) return !0;
        c = c.sourceBlock_;
        do {
            if (l == c) return !0;
            c = c.getParent()
...
// 2018/06/19 Press enter key to update input text
Blockly.FieldTextInput.prototype.onHtmlInputChange_ = function(a) {
    var b = Blockly.FieldTextInput.htmlInput_;
    13 == a.keyCode ? (a = b.value, a !== b.oldValue_ ? (b.oldValue_ = a, this.setText(a), this.validate_()) : goog.userAgent.WEBKIT && this.sourceBlock_.render(), Blockly.WidgetDiv.hide()) : 27 == a.keyCode && (this.setText(b.defaultValue), Blockly.WidgetDiv.hide())
};
...
Blockly.FieldTextInput.numberValidator = function(a) {
    ...
    return a.endsWith(".0") ? isNaN(b) ? null : String(b) + ".0" : isNaN(b) ? null : String(b)
};
// Copyright 2013 Google Inc.  Apache License 2.0
...
//Blockly.FieldDropdown.prototype.getParentType=function(a,b){var c=null;if(a.getParent()){var d=a.getParent();if("controls_for"==a.type)return null;d.type.match("procedures_callreturn")&&(d=d.getParent());if(d.type==b+"_*"&&"variables_set"==d.getParent().type||d.type==b+"_pointer_&"&&d.getParent().type==b+"_pointer_set")c=d.getParent().getVars(),c=this.getTypefromVars(c,0);else if(d.type!=b+"_pointer_&"&&d.type!=b+"_pointer_*"||!d.getParent())if(d.type!=b+"_&"&&d.type!=b+"_*"||d.getParent().type!=
//b+"_declare")if(d.type!=b+"_&"&&d.type!=b+"_*"||d.getParent().type!=b+"_set")("library_stdlib_malloc"==a.type||a.type==b+"_get")&&0<d.type.search("_set")?(d=d.getVars(),"variables_pointer"==b&&(d=d.toString().replace("* ","")),c=this.getTypefromVars(d,0)):a.type!=b+"_set"&&d.type.match("_declare")?d.getDeclare()&&(c=d.getTypes()):d.type.match("main_block")?c="int":d.type.match("procedures_return")&&(c=d.getType());else{var c=d.getParent().getVars(),e=this.getTypefromVars(c,5),c=this.getTypefromVars(c,
//0);"**"==e&&d.type==b+"_&"?c=c.replace("db",""):"*"==e&&d.type==b+"_*"&&(c="db"+c)}else e=d.getParent().getSpec(),"**"==e&&d.type==b+"_&"?c=d.getParent().getType():"*"==e&&d.type==b+"_*"&&(c=d.getParent().getType(),c="db"+c);else d.getParent().getVars()&&(c=d.getParent().getTypes())}return c};
Blockly.FieldDropdown.prototype.getParentType = function(d, c) {
    var b = null;
    if (d.getParent()) {
        var a = d.getParent();
        if ("controls_for" == d.type) return null;
        a.type.match("procedures_callreturn") && (a = a.getParent());
        if (a.type == c + "_*" && "variables_set" == a.getParent().type || a.type == c + "_pointer_&" && a.getParent().type == c + "_pointer_set") b = a.getParent().getVars(), b = this.getTypefromVars(b, 0);
        else if (a.type != c + "_pointer_&" && a.type != c + "_pointer_*" || !a.getParent())
            if (a.type != c + "_&" && a.type != c + "_*" || a.getParent().type !=
                c + "_declare")
                if (a.type != c + "_&" && a.type != c + "_*" || a.getParent().type != c + "_set")("library_stdlib_malloc" == d.type || d.type == c + "_get") && 0 < a.type.search("_set") ? (a = a.getVars(), "variables_pointer" == c && (a = a.toString().replace("* ", "")), b = this.getTypefromVars(a, 0)) : d.type != c + "_set" && a.type.match("_declare") ? a.getDeclare() && (b = a.getTypes()) : a.type.match("main_block") ? b = "int" : a.type.match("procedures_return") && (b = a.getType());
                else {
                    b = a.getParent().getVars();
                    var e = this.getTypefromVars(b, 5);
                    b = this.getTypefromVars(b,
                        0);
                    "**" == e && a.type == c + "_&" ? b = b.replace("db", "") : "*" == e && a.type == c + "_*" && (b = "db" + b)
                }
        else e = a.getParent().getSpec(), "**" == e && a.type == c + "_&" ? b = a.getParent().getType() : "*" == e && a.type == c + "_*" && (b = a.getParent().getType(), b = "db" + b);
        else try {
            a.getParent().getVars() && (b = a.getParent().getTypes())
        } catch (f) {
...
Blockly.Variables.allVariables = function(a) {
    ...
    for (var c = 0; c < b.length; c++) {
        var d = b[c].getDeclare,
            e = b[c].getParamInfo;
        if (d) {
            for (var e = d.call(b[c]), f = b[c].getTypes.call(b[c]), g = b[c].getDist.call(b[c]), h = b[c].getScope.call(b[c]), k = b[c].getPos.call(b[c]), d = b[c].getSpec.call(b[c]), l = 0; l < g.length; l++) var m = g[l];
            for (g = 0; g < f.length; g++) var n = f[g];
            for (f = 0; f < e.length; f++) var q = e[f];
            if (h)
                for (f = 0; f < h.length; f++) var p = h[f];
            e = k;
            if ("v" != m && "d" != m) var r = d;
            q && p && (a[q.toLowerCase() + "." + p.toLowerCase()] = [n, m, q, p, e, r])
        } else if (e && (d = e.call(b[c])))
            for (e = 0; e < d.length; e++) a[d[e][2].toLowerCase() + "." + d[e][3].toLowerCase()] = [d[e][0], d[e][1], d[e][2], d[e][3], d[e][4], d[e][5]]
    }
    var m = [],
        t;
    for (t in a) m.push([a[t][0], a[t][1], a[t][2], a[t][3], a[t][4], a[t][5]]);
    return m
};
...
// 2018/06/20 rename variables by scope and skip function call in main function
Blockly.Variables.renameVariablebyScope = function(f, g, h) {
    for (var b = Blockly.mainWorkspace.getTopBlocks(!1), e = 0, c, a = 0; a < b.length; a++) {
        var d = b[a].getName;
        if (d && (c = b[a].getName(), c[0] == h)) {
            e = a;
            break
        }
    }
    b = [b[e]];
    for (a = 0; a < b.length; a++) b.push.apply(b, b[a].getChildren());
    for (a = 0; a < b.length; a++)(d = b[a].renameVar) && ("Main" != c[0] || "procedures_callnoreturn" != b[a].type && "procedures_callreturn" != b[a].type) && d.call(b[a], f, g)
};
// 2018/06/20
...
Blockly.Names.equals = function(a, b) {
    return a && b ? a.toLowerCase() == b.toLowerCase() : a == b ? !0 : !1
};
...
Blockly.Structure.allStructure = function() {
    ...
    for (d = 0; d < a.length; d++)
        if (e = a[d].getStructDeclare) {
            for (var e = e.call(a[d]), f = a[d].getTypes(), f = f.toString(), e = e.toString(), g, h, k, l, m = 0; m < b.length; m++) b[m][1] == f && (h = b[m][2], g = b[m][3], k = b[m][4], l = b[m][5]);
            e = ["sn", f, e, h, g, k, l];
            c.push(e)
...
// 2018/06/20
Blockly.Procedures.FUNC_NAME = "";
Blockly.Procedures.setProcedureName = function(a) {
    Blockly.Procedures.FUNC_NAME = a
};
Blockly.Procedures.getProcedureName = function() {
    return Blockly.Procedures.FUNC_NAME
};
// 2018/06/20
...
Blockly.Procedures.flyoutCategory = function(a, b, c, d) {
    function e(e, f) {
        for (var k = 0; k < e.length; k++) {
            ...
            l.setFieldValue(e[k][1], "NAME");
            for (var m = [], n = 0; n < e[k][3].length; n++) m[n] = "ARG" + n;
            l.setProcedureParameters(e[k][3], e[k][4], e[k][5], e[k][6], m);
            l.initSvg();
            a.push(l);
            b.push(2 * c)
        }
...
Blockly.Css.CONTENT = [".blocklySvg {", "  background-color: #fff;", "  border: 1px solid #ddd;", "  overflow: hidden;", "}", ".blocklyWidgetDiv {", "  position: absolute;", "  display: none;", "  z-index: 999;", "}", ".blocklyDraggable {", "  cursor: url(media/handopen.cur) 8 5, auto;", "}", ".blocklyResizeSE {", "  fill: #aaa;", "  cursor: se-resize;", "}", ".blocklyResizeSW {", "  fill: #aaa;", "  cursor: sw-resize;", "}", ".blocklyResizeLine {", "  stroke-width: 1;", "  stroke: #888;", "}", ".blocklyHighlightedConnectionPath {",
    ...
    "}", ".blocklyWidgetDiv .goog-option-selected .goog-menuitem-checkbox,", ".blocklyWidgetDiv .goog-option-selected .goog-menuitem-icon {", "  background: url(media/sprites.png) no-repeat 0 0 !important;", "}", ".blocklyToolboxDiv {", "  background-color: #f5f5f5;", "  display: none;", "  overflow-x: visible;", "  overflow-y: auto;", "  position: absolute;", "}", ".blocklyTreeRoot {", "  padding: 4px 0;", "}", ".blocklyTreeRoot:focus {", "  outline: none;", "}", ".blocklyTreeRow {", "  line-height: 22px;",
    "  height: 22px;", "  padding-right: 1em;", "  white-space: nowrap;", "}", '.blocklyToolboxDiv[dir="RTL"] .blocklyTreeRow {', "  padding-right: 0;", "  padding-left: 1em !important;", "}", ".blocklyTreeRow:hover {", "  background-color: #e4e4e4;", "}", ".blocklyTreeIcon {", "  height: 16px;", "  width: 16px;", "  vertical-align: middle;", "  background-image: url(media/tree.png);", "}", ".blocklyTreeIconClosedLtr {", "  background-position: -32px -1px;", "}", ".blocklyTreeIconClosedRtl {", "  background-position: 0px -1px;",
...
Blockly.parseOptions_ = function(a) {
    ...
    if (b) var c = !1,
        ...
        e = !1,
        f = null;
    else f = Blockly.parseToolboxTree_(a.toolbox), c = Boolean(f && f.getElementsByTagName("category").length), d = a.trashcan, void 0 === d && (d = c), e = a.collapse, void 0 === e && (e = c);
    if (f && !c) var g = !1;
...
Blockly.setCursorHand_ = function(a) {
    if (!Blockly.readOnly) {
        var b = "";
        a && (b = "url(media/handclosed.cur) 7 3, auto");
